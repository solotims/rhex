...
double h1 = this->model->GetJoint("link_0_JOINT_1")->Position(0);
double h2 = this->model->GetJoint("link_0_JOINT_2")->Position(0);
h1 = (h1 + a1s)*1.5;
h2 = (h2 + a2s)*1.5;
std::vector<std::string> jo  = {"link_0_JOINT_1", "link_0_JOINT_2", "link_0_JOINT_3", "link_0_JOINT_4", "link_0_JOINT_5", "link_0_JOINT_6"};
for (int i = 0; i <= 2; i++){
this->model->GetJoint(jo[2*i])->SetVelocity(0, -h1);
this->model->GetJoint(jo[2*i + 1])->SetVelocity(0, -h2);
